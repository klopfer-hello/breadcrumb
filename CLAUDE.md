...
Quest data is generated by the **BreadcrumbAiQuestGenerator** tool (separate project in `../BreadcrumbAiQuestGenerator/`). The tool scrapes Wowhead, uses Claude to extract structured steps, and outputs JSON files that are converted to Lua.

### Workflow

```bash
cd ../BreadcrumbAiQuestGenerator

# Generate a quest chain (give any quest ID from the chain)
python generator.py --quest 10246 --chain

# Fix any issues in the JSON files
# Edit output/quests/XXXXX.json

# Build Lua for the addon
python build_lua.py --zone Netherstorm --mapid 1953

# Test in-game, verify coords with /bc status
```

### Manual quest research (without the tool)
...
- **BreadcrumbAiQuestGenerator** (`../BreadcrumbAiQuestGenerator/`) — Python tool that scrapes Wowhead + uses Claude to generate quest JSON files, then converts them to Lua zone files. See its own CLAUDE.md for details.
